...
const ProductDisplayPage = () => {
  ...
  <span className="text-gray-400">{data.name}</span>
        ...
        <p className="bg-green-500 text-white w-fit px-3 py-1 rounded-full flex items-center gap-2 shadow-md mb-3">
          <motion.div
            animate={{ y: [0, 3, 0] }} // Moves down and up
        ...
        <h2 className="text-lg font-semibold lg:text-3xl w-fit px-2">
          {data.name}
        </h2>
        <p className="w-fit px-2">{data.unit}</p>
        <Divider />
        <div>
          <p className=" w-fit px-2  text-green-600">Price</p>
          <div className="flex items-center gap-2 lg:gap-4">
            ...
            {data.discount && (
              <p className="italic font-semibold text-green-600 lg:text-xl">
                {data.discount}%{" "}
                <span className="italic font-semibold text-green-600 lg:text-xl">
                  Off
                </span>
              </p>
            )}
        ...
        {data.stock === 0 ? (
          <p className="text-lg text-red-500 w-fit px-2 my-2">Out of Stock</p>
        ) : (
          // <button className='my-4 px-4 py-1 bg-green-600 hover:bg-green-700 text-white rounded'>Add</button>
          <div className="my-4 w-fit px-1">
            <AddToCartButton data={data} />
          </div>
        )}

        <h2 className="font-semibold">Why shop from GrocyGo? </h2>
        <div>
